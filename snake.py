...
class Snake:
    ...
    def grow(self):
        ...
        None --> None
        """
        store_loc = self._body[-1]
        for _ in range(self._growth_rate):
            if store_loc not in self._gc.deadly_points:
                self._body.append(SnakeSquare(
                    store_loc._x, store_loc._y, store_loc._dot_color))
                self._gc.update()
            else:
                self._gc.update()
    ...
    def _update_move(self, do_move):
        ...
        if (do_move and len(self._body)):
            ...
            # solution to problem 1 was to set range step to reverse -1
